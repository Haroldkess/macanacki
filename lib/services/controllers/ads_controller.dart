  ...
  static Future<void> sendAdRequeest(
    ...
    if (isDone) {
      ...
      if (user.userProfileModel.country!.toLowerCase() ==
          "Nigeria".toLowerCase()) {
        // ignore: use_build_context_synchronously
        //log("${data.postId}, ${data.planId}");
        await PlanController.promotePostLink(
            context, data.postId, ware.selected.id);
      } else {
        List<Package> verificationPackages =
            await PayExt.fetchPackagesFromOfferings(identifier: 'promotion');
